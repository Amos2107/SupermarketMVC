...
function getConfig(req, orderId, outTradeNo) {
  ...
  const brandName = (process.env.PAYPAL_BRAND_NAME || 'AmosMart').trim();
  ...
  const returnUrl = `${appBaseUrl}/orders/${encodeURIComponent(orderId)}/pay/finish${qs}`;
  const cancelUrl = `${appBaseUrl}/orders/${encodeURIComponent(orderId)}/pay${qs ? `${qs}&cancel=paypal` : '?cancel=paypal'}`;
    ...
    const req = https.request(
      ...
      (res) => {
        ...
        res.on('end', () => {
          ...
          if (!ok) return reject(new Error(`PayPal HTTP ${res.statusCode}: ${raw.slice(0, 300)}`));
          try {
